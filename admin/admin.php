...
<a href="tambah-wisata.php">TAMBAH WISATA</a>
	<table border="1px">
		<thead>
			<tr>
				<th>No</th>
		...
		<tbody>
			...
			$acuan = $_POST['acuan'];
			$sql = mysqli_querY($conn,"SELECT a.id,a.nama,a.gambar,a.isi,a.lokasi,a.jambuka,b.kategori,c.subkategori from tbl_wisata a left join tbl_kategori b on a.kategori = b.id left join tbl_subkategori c on a.subkategori = c.id where a.nama like '%$acuan%' or a.isi like '%$acuan%' or a.lokasi like '%$acuan%' or a.jambuka like '%$acuan%' or b.kategori like '%$acuan%' or c.subkategori like '%$acuan%'");
			}else{
			$sql = mysqli_querY($conn,"SELECT a.id,a.nama,a.gambar,a.isi,a.lokasi,a.jambuka,b.kategori,c.subkategori from tbl_wisata a left join tbl_kategori b on a.kategori = b.id left join tbl_subkategori c on a.subkategori = c.id");
			}
			...
			<tr>
				<td><?=$no?></td>
				<td><?=$data['nama']?></td>
				<td><img src="../images/<?=$data['gambar']?>"></td>
				<td class="isi"><?=nl2br($data['isi'])?></td>
				<td><?=$data['lokasi']?></td>
				<td><?=$data['jambuka']?></td>
				<td><?=$data['kategori']?></td>
				<td><?=$data['subkategori']?></td>
				<td><a href="edit-wisata.php?id=<?=$data['id']?>">Edit</a><br><a href="delete-wisata.php?id=<?=$data['id']?>">Delete</a></td>
			</tr>
			<?php
			$no++;
			}
